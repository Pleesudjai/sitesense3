...
- For zoning_lookup: extract the city name from parcel_lookup's "address" field (e.g., "1435 N DORSEY LN   TEMPE  85288" → city="TEMPE") and pass as the city argument. The tool covers Tempe, Phoenix, Mesa, Scottsdale, Gilbert, Chandler (boundary-detect only — Chandler does not publish public zoning REST, so the tool returns jurisdiction='Chandler' with confidence='unknown' and the user must verify manually), and Maricopa County unincorporated (rural areas). If the parcel is in another Maricopa city (Goodyear, Avondale, Surprise, Peoria, Glendale, Buckeye, Apache Junction, etc.) the tool returns jurisdiction="Outside covered jurisdictions" — surface that to the user as a known coverage gap and recommend they call the relevant city planning department.
