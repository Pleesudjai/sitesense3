...
export const sitesenseMcpServer = createSdkMcpServer({
  ...
  tools: [
    ...
    tool(
      'zoning_lookup',
      'Look up the zoning district at a point (lat/lon) across covered Maricopa jurisdictions. Real dimensional data (setbacks, height, density): Tempe, Phoenix, Mesa, Scottsdale, Gilbert, Glendale, Maricopa County unincorporated. Hint-only stubs (no public zoning REST exists; the tool confirms jurisdiction and directs the user to the relevant planning department): Chandler, Goodyear, Avondale, Surprise, Peoria. Pass parcel_lookup PHYSICAL_CITY as the optional `city` hint. Returns the GIS-confirmed zoning code, district name, jurisdiction, dimensional standards, and a confidence rating per value. If the point is outside all 12 jurisdictions (likely Buckeye, Apache Junction, El Mirage, Tolleson, Fountain Hills, Litchfield Park, Paradise Valley, etc., or outside Maricopa County) the tool returns jurisdiction="Outside covered jurisdictions" with cities_tried.',
