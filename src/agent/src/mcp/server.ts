...
export const sitesenseMcpServer = createSdkMcpServer({
  ...
  tools: [
    ...
    tool(
      'zoning_lookup',
      'Look up the zoning district at a point (lat/lon) across covered Maricopa cities: Tempe, Phoenix, Mesa, Scottsdale, Gilbert. Returns the GIS-confirmed zoning code, district name, jurisdiction, and dimensional standards (min lot size, front/side/rear setbacks in ft, max height in ft, max density in du/acre) from each city\'s zoning ordinance. Each value carries a confidence rating; use them for first-pass envelope math but always surface the note field. Pass the parcel_lookup PHYSICAL_CITY value as the optional `city` hint to skip the parallel scan. If the point is outside all covered cities (likely Maricopa County unincorporated or Chandler/Goodyear/Avondale/Surprise/Peoria/etc.) the tool returns jurisdiction="Outside covered jurisdictions" with cities_tried so the agent can tell the user.',
